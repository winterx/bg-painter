var Placement = function ( width, height, depth, xunit, yunit, zunit ) {
    this.width = width;
    this.height = height;
    this.depth = depth;
    this.xunit = xunit || 20;
...
Placement.prototype.init = function() {
    ...
    for( var k=0; k<this.znum; ++k ) {
        for( var i=0; i<this.ynum; ++i ) {
            for( var j=0; j<this.xnum; ++j ) {
                this.matrix.push( new Pos( j*this.xunit-this.width/2, i*this.yunit-this.height/2, k*this.zunit+this.depth/2 ) );
            }
        }
    }
};

Placement.prototype.select = function() {   
